...
def parse_resume(request: ParseRequest) -> StructuredResume:
    """Download, clean, and structure a resume PDF."""
    raw_text = extract_text_from_url(request.fileUrl)
    cleaned_text = clean_resume_text(raw_text)
    resume_data = preprocess_resume_text(cleaned_text, request.fileName)
    return structure_resume(resume_data, api_key=request.geminiApiKey)
