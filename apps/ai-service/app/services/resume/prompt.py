...
- **Skills with Categories**: Extract skills grouped by their categories if the resume categorizes them (e.g., "Languages", "Frameworks & Libraries", "Databases", "Cloud & DevOps", "Tools", "Platforms", "Soft Skills", etc.). Preserve the category names as written in the resume. Each skill group in the `skills` array must contain the `category` name and an array of individual `skills`. If skills in the resume are not explicitly categorized, group them into relevant categories based on their type, or group them under a general category such as "Technical Skills".
