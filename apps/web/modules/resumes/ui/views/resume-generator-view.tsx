...
import { ChevronLeft, Download, FileText, Loader2, Plus, Trash2 } from "lucide-react";
...
const ResumePreview = ({ resume }: { resume: ResumeData }) => {
  ...
  return (
    <article className="min-h-[800px] w-full bg-white px-8 py-9 text-slate-800 shadow-[0_16px_50px_rgba(15,23,42,0.12)] sm:px-12 sm:py-12" aria-label="Resume preview">
      ...
      <PreviewSection title="Summary" visible={Boolean(text(resume.summary))}><p>{resume.summary}</p></PreviewSection>
      <PreviewSection title="Experience / Contribution" visible={Boolean(resume.experience?.length)}>
      ...
      <PreviewSection title="Tech Skills / Stack" visible={Boolean(skills.length)}>
        {skills.map((skill, index) => typeof skill === "string" ? <p key={index}>{skill}</p> : <p key={index}>{skill.category ? <strong>{skill.category}: </strong> : null}{skill.skills.join(", ")}</p>)}
      </PreviewSection>
    ...
    <div className="mx-auto flex w-full max-w-[1600px] flex-col gap-5">
      ...
      <div className="grid min-h-[calc(100vh-13rem)] gap-5 xl:grid-cols-[minmax(360px,0.82fr)_minmax(500px,1.18fr)]">
        <div className="overflow-hidden rounded-sm border border-border/70 bg-card xl:max-h-[calc(100vh-13rem)] xl:overflow-y-auto">
          <div className="sticky top-0 z-10 border-b border-border/60 bg-card px-4 py-3"><p className="text-xs font-semibold">Resume content</p><p className="text-[11px] text-muted-foreground">Changes stay in this editor until you generate.</p><div className="mt-3 grid gap-2 sm:grid-cols-2"><label className="grid gap-1"><span className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Resume template</span><select value="standard" disabled className="h-8 w-full rounded-sm border border-border bg-background px-2 text-xs outline-none opacity-90 cursor-default"><option value="standard">Standard (Summary → Experience → Projects → Tech Skills → Education)</option></select></label><label className="grid gap-1"><span className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Load parsed resume</span><select value={selectedSourceId} onChange={(event) => selectSource(event.target.value)} className="h-8 w-full rounded-sm border border-border bg-background px-2 text-xs outline-none focus-visible:ring-1 focus-visible:ring-ring"><option value="sample">Sample resume</option>{sourceResumes.map((source) => <option value={source.id} key={source.id}>{source.title}</option>)}</select></label></div>{sourceResumes.length === 0 ? <p className="mt-1 text-[10px] text-muted-foreground">No parsed resumes found. Showing the sample.</p> : null}</div>
